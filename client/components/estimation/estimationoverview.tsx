...
export default function EstimateDetailPage() {
  ...
  const [sectionForm, setSectionForm] = useState({ name: '', order: 0, category: 'DIRECT' });
  ...
  const handleCreateSection = async () => {
    try {
      ...
      setSectionForm({ name: '', order: 0, category: 'DIRECT' });
      loadEstimate();
    } catch (error) {
          ...
          <Tabs defaultValue="sections" className="space-y-4">
            ...
            <TabsContent value="sections" className="space-y-4">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <Select
                          ...
                          onValueChange={(value) => setSectionForm({ ...sectionForm, category: value as 'DIRECT' | 'INDIRECT' })}
                        >
                          <SelectTrigger>
              ...
              <div className="space-y-8">
                {['DIRECT', 'INDIRECT'].map((category) => {
                  const sections = estimate.sections.filter(s => 
                    category === 'DIRECT' ? (s.category === 'DIRECT' || !s.category) : s.category === 'INDIRECT'
                  );
                  
                  return (
                    <div key={category}>
                      <h3 className="text-xl font-semibold mb-4 flex items-center gap-2">
                        {category === 'DIRECT' ? <Building className="h-5 w-5" /> : <Wrench className="h-5 w-5" />}
                        {category === 'DIRECT' ? 'Direct Costs' : 'Indirect Costs'}
                      </h3>
                      {sections.length === 0 && (
                        <p className="text-muted-foreground italic mb-4">No {category.toLowerCase()} cost sections added.</p>
                      )}
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <h3 className="font-semibold text-lg mb-2">Direct Costs</h3>
                        <p className="text-2xl font-bold">₹{estimate.totalDirectCost?.toLocaleString() || 0}</p>
                      </div>
                      <div>
                        <h3 className="font-semibold text-lg mb-2">Indirect Costs</h3>
                        <p className="text-2xl font-bold">₹{estimate.totalIndirectCost?.toLocaleString() || 0}</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-center">
                        <span>Subtotal (Direct + Indirect)</span>
                        <span className="font-semibold">₹{(Number(estimate.totalDirectCost || 0) + Number(estimate.totalIndirectCost || 0)).toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span>Overhead ({estimate.overheadPercentage}%)</span>
                        <span>₹{((Number(estimate.totalDirectCost || 0) + Number(estimate.totalIndirectCost || 0)) * (estimate.overheadPercentage / 100)).toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span>Profit ({estimate.profitPercentage}%)</span>
                        <span>₹{((Number(estimate.totalDirectCost || 0) + Number(estimate.totalIndirectCost || 0)) * (estimate.profitPercentage / 100)).toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span>Contingency ({estimate.contingencyPercentage}%)</span>
                        <span>₹{((Number(estimate.totalDirectCost || 0) + Number(estimate.totalIndirectCost || 0)) * (estimate.contingencyPercentage / 100)).toLocaleString()}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-end">
                      <div className="text-right">
                        ...
                        <p className="text-3xl font-bold text-primary">₹{estimate.totalCost.toLocaleString()}</p>
                      </div>
                    </div>
                  </div>
                </CardContent>
